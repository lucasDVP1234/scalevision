...
function filterVideos(category) {
    ...
    const injectAndAnimateIn = () => {
        ...
        data.videos.forEach(item => {
            // CHANGEMENTS ICI :
            // 1. Les événements onmouseover/out sont sur la DIV parente (la carte)
            // 2. Ajout de onclick="openFullscreen(this)"
            // 3. Ajout de pointer-events-none sur les overlays pour laisser passer la souris
            grid.innerHTML += `
                <div class="portfolio-card group cursor-pointer opacity-0 translate-y-4 rounded-3xl border border-white/10 bg-black overflow-hidden relative aspect-[9/16] transition-all duration-500"
                     ...
                     onclick="openFullscreen(this.querySelector('video'))">
                    ...
                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 scale-0 group-hover:scale-100 pointer-events-none">
...
// --- 4. NOUVELLE FONCTION POUR LE PLEIN ÉCRAN ---
function openFullscreen(videoElem) {
    if (!videoElem) return;
    
    // On remet le son quand on clique

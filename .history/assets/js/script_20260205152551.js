...
function filterVideos(category) {
    ...
    const injectAndAnimateIn = () => {
        ...
        data.videos.forEach(item => {
            // ICI C'EST LA PARTIE MODIFIÉE POUR LE SON 👇
            grid.innerHTML += `
                <div class="portfolio-card group cursor-pointer opacity-0 translate-y-4 rounded-3xl border border-white/10 bg-black overflow-hidden relative aspect-[9/16] transition-all duration-500">
                    ...
                    <video 
                        ...
                        loop 
                        playsinline
                        onmouseover="this.muted=false; this.volume=1;" 
                        onmouseout="this.muted=true;"
                        onclick="toggleSound(this)"
                    ></video>
                    
                    <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-60 group-hover:opacity-0 transition-opacity pointer-events-none"></div>
                    
                    <div class="absolute bottom-0 left-0 w-full p-6 translate-y-4 group-hover:translate-y-0 transition-transform duration-300 pointer-events-none group-hover:opacity-0">

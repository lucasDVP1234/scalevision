...
// Fonction de filtre (Portfolio) - VERSION AVEC SON
// Fonction de filtre (Portfolio) - MODE PERFORMANCE (Pas d'autoplay)
function filterVideos(category) {
    ...
    buttons.forEach(btn => {
        ...
        if(btn.getAttribute('data-filter') === category) {
            btn.classList.add('active', 'bg-scalePink', 'text-white');
        } else {
            btn.classList.remove('bg-scalePink', 'text-white');
        }
    });
        ...
        data.videos.forEach(item => {
            grid.innerHTML += `
                <div class="portfolio-card group cursor-pointer opacity-0 translate-y-4 rounded-3xl border border-white/10 bg-black overflow-hidden relative aspect-[9/16] transition-all duration-500">
                    ...
                    <div class="absolute inset-0 bg-black/40 group-hover:bg-transparent transition-colors duration-300 pointer-events-none"></div>
                    
                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-white/10 backdrop-blur-sm border border-white/30 rounded-full flex items-center justify-center group-hover:opacity-0 group-hover:scale-150 transition-all duration-300 pointer-events-none">
                        <i class="fas fa-play text-white text-2xl ml-1"></i>
                    </div>

                    <div class="absolute bottom-0 left-0 w-full p-6 translate-y-0 group-hover:translate-y-4 group-hover:opacity-0 transition-all duration-300 pointer-events-none">
                    ...
                    <div class="absolute top-4 right-4 bg-white/20 backdrop-blur-md p-2 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 scale-0 group-hover:scale-100 pointer-events-none">
